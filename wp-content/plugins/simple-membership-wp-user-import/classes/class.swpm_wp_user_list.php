...
class SwpmWpUserList extends WP_List_Table {
    ...
    function column_default($item, $column_name) {
        switch($column_name){
            case 'membership_level':
                return sprintf('<select name="membership_level[%s]">', $item["ID"] ) . $this->membership_level_dropdown . '</select>';
            case 'subscription_starts':
                return sprintf('<input type="date" class="date_field" name="subscription_starts[%s]" value="%s" />',$item["ID"], date('Y-m-d'));
            case 'account_state':
                return sprintf('<select name="account_state[%s]">', $item["ID"] ) . BUtils::account_state_dropdown() . '</select>';
            case 'preserve_role':
    ...
    function prepare_items() {
        ...
        $orderby = !empty($_GET["orderby"]) ? strip_tags($_GET["orderby"]) : 'ASC';
        $order = !empty($_GET["order"]) ? strip_tags($_GET["order"]) : '';
